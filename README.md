...
Yahoo historical validation runs through the `validation` profile when `YAHOO_ENABLED=true` and `YAHOO_HISTORICAL_VALIDATION_ENABLED=true`. It stores Yahoo source-specific daily bars, preserves Yahoo `Close` and `Adj Close` separately, compares close and volume against Massive/Polygon only under the active compatibility policy, records discrepancies by comparison rule version, and never changes canonical bar selection.
